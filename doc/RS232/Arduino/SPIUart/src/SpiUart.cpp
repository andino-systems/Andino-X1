...
void SpiUartDevice::writeRegister(byte registerAddress, byte data) {
  ...
  select();
  SPI.transfer(registerAddress|uartChannel);
  SPI.transfer(data);
  deselect();
}
...
byte SpiUartDevice::readRegister(byte registerAddress) {
  ...
  char result;
  select();
  SPI.transfer(SPI_READ_MODE_FLAG | registerAddress | uartChannel);
...
void SpiUartDevice::write(const uint8_t *buffer, size_t size) {
   ...
   */
  select();
  transfer(THR | uartChannel); // TODO: Change this when we modify register addresses? (Even though it's 0x00.) 
